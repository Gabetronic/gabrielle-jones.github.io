 ...
 * 3. Comparison operators compare the left and right operands and return a Boolean value based on whether the comparision is true or false.
 * 4. Logical operators also evaluate each operand and coerce them to a Boolean value; however, they are limited in scope to 3 types of 
 * logical evaluations: && (aka AND), || (aka OR), and ! (aka NOT).
 * 5. Unary operators only have one operand to evaluate. There are many unary operators, but this project covers only three: typeof, 
 * ! (aka logical NOT), and - (aka unary negation)
...
var name = character.lastName === "Done" ? true : false; 
console.log(name);   //prints true